...
$username_err = $password_err = $email_err = $phone_err = $age_err = $cnic_err = "";
...
if (empty(trim($_POST["cnic"]))) {
...
} elseif (!preg_match('/^[0-9]{13}$/', trim($_POST["cnic"]))) {
    $cnic_err = "Invalid CNIC format. CNIC must be exactly 13 digits without dashes.";
} else {
    ...
    if (mysqli_stmt_num_rows($stmt) == 1) {
        $cnic_err = "This CNIC number is already taken.";
    }
}
    ...
    if (empty($username_err) && empty($password_err) && empty($email_err) && empty($phone_err) && empty($age_err) && empty($cnic_err)) {
        ...
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "ssssis", $param_username, $param_password, $param_email, $param_phone, $param_age, $param_cnic);
    ...
    <div class="container mt-5">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <div class="form-group">
                ...
                <input type="text" name="cnic" class="form-control <?php echo (!empty($cnic_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $cnic; ?>">
